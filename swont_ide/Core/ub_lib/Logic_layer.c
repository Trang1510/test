...
ColorType get_color_type(const char* command)
{
    ...
    if (strncmp(command, "lichtblauw", strlen("lichtblauw")) == 0) return CMD_UNKNOWN;
    if (strncmp(command, "groen", strlen("groen")) == 0) return VGA_COLOUR_GREEN;
    if (strncmp(command, "lichtgroen", strlen("lichtgroen")) == 0) return CMD_UNKNOWN;
    if (strncmp(command, "cyaan", strlen("cyaan")) == 0) return VGA_COLOUR_CYAN;
    if (strncmp(command, "lichtcyaan", strlen("lichtcyaan")) == 0) return CMD_UNKNOWN;
    if (strncmp(command, "zwart", strlen("zwart")) == 0) return VGA_COLOUR_BLACK;
    if (strncmp(command, "lichtrood", strlen("lichtrood")) == 0) return CMD_UNKNOWN;
    if (strncmp(command, "magenta", strlen("magenta")) == 0) return VGA_COLOUR_MAGENTA;
    if (strncmp(command, "lichtmagenta", strlen("lichtmagenta")) == 0) return CMD_UNKNOWN;
    if (strncmp(command, "bruin", strlen("bruin")) == 0) return CMD_UNKNOWN;
    if (strncmp(command, "geel", strlen("geel")) == 0) return VGA_COLOUR_YELLOW;
    if (strncmp(command, "grijs", strlen("grijs")) == 0) return CMD_UNKNOWN;
